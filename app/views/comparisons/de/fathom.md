...
**Server-Side-SDK + Custom Events vom Backend.** Pro-Tier hat ein Ruby-Gem, npm-Package, Pixel-Endpoint und das `record_event` MCP-Tool — du kannst Webhook-Deliveries, Server-Side-Conversions, Cron-Jobs tracken, alles, was der JS-Tracker nie sieht. Fathom ist JS-only auf Cloud (die selbst-gehostete Lite-Version ist anders).
...
Viele Teams behalten Fathom auf der Marketing-Hauptsite (für's Dashboard) und nehmen uns auf Side-Projects und interne Tools (für den Chat-Workflow). Es gibt keine Regel, dass eins gewinnen muss.
