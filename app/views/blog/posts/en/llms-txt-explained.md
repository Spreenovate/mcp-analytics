...
If you've seen blog posts in the last twelve months pitching `llms.txt` as "the new robots.txt for AI", they're wrong on the substance. `llms.txt` is something else entirely — and worth getting right because the alternative (your site getting summarized poorly by AI search engines that *can't* parse your nav) is real.
...
Proposed by Jeremy Howard ([Answer.AI](https://answer.ai)) in September 2024 and tracked at [llmstxt.org](https://llmstxt.org), it solves a specific problem: when an LLM grounds an answer in your site, it has a limited context window. It cannot read your entire site map, fetch every linked page, parse JavaScript-rendered nav, and synthesize a coherent answer in the few seconds the user is waiting. So it picks the wrong pages, misses your pricing, summarizes outdated marketing copy, or just hallucinates.
...
**Two sub-conventions** are worth knowing:
...
2. **Section headings reflect user intent**. "Product surface", "MCP tool catalog", "Content" — they're how a *user* would categorize what to read, not how *we* categorize internally.
...
| [cursor.com/llms.txt](https://cursor.com/llms.txt) | Two-tier structure: product features + docs, clear which to read for which question |
...
This is the question that determines whether you should bother. As of the date of this post:
...
- **Google AI Overviews** — Google has not committed publicly to reading `llms.txt`. They use their existing crawl + Knowledge Graph instead.

Practical takeaway: shipping `llms.txt` has **non-zero return** today and **likely growing return** through 2026 as more clients adopt. The cost is one markdown file — there's no reason to skip it.
...
This isn't required, but a few crawlers pick it up faster if you do:
...
3. **`robots.txt` AI-crawler allowance.** If you `Disallow: /` for `GPTBot`, `ClaudeBot`, or `PerplexityBot`, you're invisible in those engines. Common mistake: people who installed a "block AI crawlers" plugin in 2024 to protest training and forgot to undo it when they later wanted to be in ChatGPT's answer set.

If you only do one of the three, do #3 (allow AI crawlers). If you do two, do #3 + #2. The combined cost is half an hour and lasts forever.
...
`llms.txt` is two years old as a proposal. Expect three things in the next 12-18 months:
...
- **Tooling.** Static-site generators will ship `llms.txt` plugins. Vercel/Netlify will likely auto-generate one from sitemap + meta-descriptions.

For now: write yours manually, keep it ~50 lines, and revisit every quarter.
...
For mcp-analytics — the product we're building — both are in production. The MCP server is the product; `llms.txt` and the AI-friendly `robots.txt` are how new users discover that the product exists.
